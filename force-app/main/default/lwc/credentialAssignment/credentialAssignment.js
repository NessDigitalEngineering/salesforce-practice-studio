...
export default class CredentialAssignment extends LightningElement {
          ...
          if (tmpCredList.length > 0) {
            tmpCredList.push(
              this.addCredentials(
                rec.recId,
                rec.recName,
                tmpCredList[tmpCredList.length - 1].dueDate
              )
            );
          } else {
  ...
  addCredentials(credId, credName, previousCredDate) {
    try {
      ...
      credRec.dueDate =
        previousCredDate === null
          ? this.addDaysToDate(null)
          : this.addDaysToDate(previousCredDate);
  ...
  rowactionDelete(event) {
    ...
    if (this.constant.DEL_CONFIRM) {
      ...
      for (let cred in tmpList) {
        if (
          tmpList[cred].credName === event.detail.row.credName &&
          tmpList[cred].status === ""
        ) {
          ...
          this.template
            ...
            .removeCredentials(tmpList[cred].credName);
        } else if (tmpList[cred].status === "") {
          let index = updatedList.findIndex(
            (x) => x.credName === tmpList[cred].credName
          );
          updatedList.splice(
            index,
            1,
