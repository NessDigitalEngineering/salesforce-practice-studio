...
const columns = [
  {
    label: "Exam Attempt Id",
    fieldName: "Name"
  },
  {
    label: "Credential Name",
    fieldName: "Credential__c"
  },
  {
    label: "Submitted By",
    fieldName: "User__c"
...
export default class VoucherAssignment extends LightningElement {
  ...
  /*
        ...
        */
  connectedCallback() {
    this.getVoucherApprovedUsers();
  }

  getVoucherApprovedUsers() {
    getVoucherApprovedUsers()
      .then((res) => {
        this.lstCred = res;
  ...
  showModalBox() {
    ...
    if (selectedRec.length > 0) {
      ...
      selectedRec.forEach((currentItem) => {
        ...
        this.credCost =
          currentItem.User_Credential__r.Credential__r.Registration_fee__c;
        credName =
          currentItem.Credential__c +
          " $" +
          currentItem.User_Credential__r.Credential__r.Registration_fee__c;
    ...
    updateExamRecords({
      ...
      .then((response) => {
        ...
        updateVouchersStatus({
          voucherId: this.exmVoucher,
          voucherStatus: "Assigned",
          examAttemptId: this.parentID
        })
          .then((response) => {
            this.isShowModal = false;
            this.getVoucherApprovedUsers();
  ...
  handleSortdata(event) {
    this.template.querySelector(
      'lightning-datatable[data-id="exmVoucher"]'
    ).selectedRows = [];
    // field name
  ...
  sortData(fieldname, direction) {
    try {
      ...
      this.lstVoucher = [];
      this.lstVoucher.push(...parseData);
    } catch (error) {
      console.error(error);
    }
  }
