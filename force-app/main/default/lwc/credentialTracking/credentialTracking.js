...
export default class CredentialTracking extends LightningElement {
  ...
  @wire(getPicklistValues, {
    recordTypeId: "$userCredentialMetadata.data.defaultRecordTypeId",
    fieldApiName: STATUS
  })
  ...
  handleSuccess(event) {
    ...
    let index = this.userCredentialsData.findIndex(
      (x) => x.Id === event.detail.recId
    );
    ...
    if (this.statusValuesReady) {
      this.totalUserCredentials.forEach((e) => {
        if (e.Status__c && e.Status__c != "Completed") {
          e.nextStatusLbl =
            this.statusValues[this.statusValues.indexOf("" + e.Status__c) + 1] +
            " >";
          e.nextStatus =
            this.statusValues[this.statusValues.indexOf("" + e.Status__c) + 1];
